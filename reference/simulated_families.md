...
simulation_study.Rmd and using_penetrance.Rmd The user must specify the
`pedigree` argument as a data frame which contains the family data (see
`test_fam`). The family data must be in the correct format with the
following columns:
